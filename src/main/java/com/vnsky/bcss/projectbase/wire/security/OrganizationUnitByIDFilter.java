...
@Slf4j
@Order()
public class OrganizationUnitByIDFilter extends OncePerRequestFilter {
    private static final String API_KEY_HEADER = "x-api-key";

    @Value("${third-party.admin.apikey}")
    private String webViewApiKey;

    @Value("${application.internal-path.organization-unit-by-id}")
    private String webviewUrlPattern;
    ...
    protected void doFilterInternal(HttpServletRequest request, @NotNull HttpServletResponse response, @NotNull FilterChain filterChain) throws ServletException, IOException {
        ...
        if(!request.getRequestURI().contains(webviewUrlPattern)) {
            log.info("[FILTER]: {}", webviewUrlPattern);
            filterChain.doFilter(request, response);
            return;
        }

        if(apiKey == null || !apiKey.equals(webViewApiKey)){
